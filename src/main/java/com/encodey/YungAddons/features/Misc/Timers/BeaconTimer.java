...
public class BeaconTimer {
    ...
    public void onChat(final ClientChatReceivedEvent event) {
        ...
        if(message.startsWith("NICE! SLAYER BOSS ") || message.contains("SLAYER QUEST FAILED!")) {
            activated = false;
            running = false;
            runningact = false;
            holdingbeacon = false;
            ...
            try {
                if(event.type == RenderGameOverlayEvent.ElementType.ALL) {
                    ...
                    if (!beaconPlaced) {
                        ...
                        if(runningact) {
                            textbeac = EnumChatFormatting.LIGHT_PURPLE + UtilsDanker.getTimeBetween(timeNow, beaconTime);
                            ...
                            FontUtils.drawCenteredString("Beacon: " + String.format("%.3fs", textbeac.replace("0m","")), width/ 2.0f / scale, (height / 2.8f - 15.0f) / scale, 0xD62440);
                            new Thread(() -> {
                                try {
                                    Thread.sleep(5000L);
                                    beaconPlaced = false;
