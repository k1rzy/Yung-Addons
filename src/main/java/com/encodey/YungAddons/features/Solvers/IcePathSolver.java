...
import com.encodey.YungAddons.utils.MathUtils.PathUtils;
...
public class IcePathSolver {
    ...
    static List<PathUtils.Point> route = new ArrayList<>();
    public static int SILVERFISH_LINE_COLOUR = 0x00;
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        EntityPlayerSP player = mc.thePlayer;
            if (Config.icepathsolver && DungeonChestESP.dungeonStarted && world != null && player != null) {
                new Thread(() -> {
                                ...
                                for (BlockPos blockPos : blocks) {
                                    if (world.getBlockState(blockPos).getBlock() == Blocks.chest && world.getBlockState(blockPos.add(0, 2, 0)).getBlock() == Blocks.hopper) {
                                        ...
                                        if (!prevInSilverfishRoom) {
                                            ...
                                            PathUtils.Point silverfishPoint = null;
                                                    ...
                                                    for (int row = chest.getZ() + 3, xIteration = 0; xIteration < 19; row++, xIteration++) {
                                                        for (int column = chest.getX() - 9, yIteration = 0; yIteration < 19; column++, yIteration++) {
                                                            if (world.getBlockState(new BlockPos(column, 67, row)).getBlock() != Blocks.air) {
                                                                ...
                                                                silverfishPoint = new PathUtils.Point(xIteration, yIteration);
                                                            }
                                                        }
                                                    }
                                                    break;
                                            ...
                                            List<PathUtils.Point> endPoints = new ArrayList<>();
                                            for (int column = 0; column < 19; column++) {
                                                if (board[0][column] != 'X') endPoints.add(new PathUtils.Point(0, column));
                                            }

                                            route = PathUtils.solve(board, silverfishPoint, endPoints);
                                        }
                                    }
                                }
                            }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        if (Config.icepathsolver && DungeonChestESP.dungeonStarted && route != null && route.size() > 0 && chest != null) {
            for (int i = 0; i < route.size() - 1; i++) {
                ...
                UtilsDanker.draw3DLine(pos1, pos2, SILVERFISH_LINE_COLOUR, 5, true, event.partialTicks);
            }
        }
    }
