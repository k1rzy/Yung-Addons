...
public class CreeperSolver {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        EntityPlayerSP player = mc.thePlayer;
            if (Config.creepersolver && DungeonChestESP.dungeonStarted && world != null && player != null) {
                ...
                if (creepers.size() > 0 && !creepers.get(0).isInvisible()) { // Don't show Wither Cloak creepers
                    ...
                    for (BlockPos blockPos : blocks) {
                        ...
                        if (block == Blocks.sea_lantern || block == Blocks.prismarine) {
                            ...
                            BlockPos oppositeBlock = UtilsDanker.getFirstBlockPosAfterVectors(mc, startBlock, creeperLocation, 10, 20);
                            BlockPos endBlock = UtilsDanker.getNearbyBlock(mc, oppositeBlock, Blocks.sea_lantern, Blocks.prismarine);
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        if (Config.creepersolver && drawCreeperLines && !creeperLines.isEmpty()) {
            for (int i = 0; i < creeperLines.size(); i++) {
                UtilsDanker.draw3DLine(creeperLines.get(i)[0], creeperLines.get(i)[1], CREEPER_COLOURS[i % 10], 2, true, event.partialTicks);
            }
        }
    }
