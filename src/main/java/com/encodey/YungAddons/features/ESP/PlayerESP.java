            ...
            for(Entity player : (Minecraft.getMinecraft().theWorld.loadedEntityList)) {
                if(player instanceof EntityPlayer) {
                    if(player != null) {
                        ...
                        AxisAlignedBB aabb = new AxisAlignedBB(player.posX - 0.5, player.posY - 2, player.posZ - 0.5, player.posX + 0.5, player.posY + 2, player.posZ + 0.5);
                        
                        UtilsDanker.draw3DBox(aabb, 0xD64FC8, event.partialTicks);
                    }
                }
            }
        }
