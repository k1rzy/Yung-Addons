...
public class Reparty {
    ...
    public static boolean dungeonEnded;
    ...
    public void chatCheck(final ClientChatReceivedEvent event) {
        String message = event.message.getUnformattedText();
        if(message.startsWith("The Catacombs - Floor ") && activated && Config.autoRP) {
            dungeonEnded = true;
        }
    }
    ...
    public void onChat(ClientChatReceivedEvent event) {
        ...
        // Getting party
        if (RepartyCommand.gettingParty && dungeonEnded) {
            if (message.contains("-----")) {
            ...
            }else if (message.startsWith("Party M") || message.startsWith("Party Leader") && dungeonEnded){
                ...
                if (party_start.matches() && Integer.parseInt(party_start.group(1)) == 1) {
                    player.addChatMessage(new ChatComponentText(YungAddons.ERROR_COLOUR + "You cannot reparty yourself."));
                    RepartyCommand.partyThread.interrupt();
                } else if (leader.matches() && !(leader.group(1).equals(player.getName()))) {
                    player.addChatMessage(new ChatComponentText(YungAddons.ERROR_COLOUR + "You are not party leader."));
                    RepartyCommand.partyThread.interrupt();
                } else {
                    while (members.find()) {
        ...
        // Disbanding party
        if (RepartyCommand.disbanding  && dungeonEnded) {
            if (message.contains("-----")) {
                switch (RepartyCommand.Delimiter) {
                    case 0:
        ...
        // Inviting
        if (RepartyCommand.inviting  && dungeonEnded) {
            if (message.contains("-----")) {
                switch (RepartyCommand.Delimiter) {
                    case 1:
                        event.setCanceled(true);
        ...
        // Fail Inviting
        if (RepartyCommand.failInviting  && dungeonEnded) {
            if (message.contains("-----")) {
                switch (RepartyCommand.Delimiter) {
                    case 1:
                        event.setCanceled(true);
