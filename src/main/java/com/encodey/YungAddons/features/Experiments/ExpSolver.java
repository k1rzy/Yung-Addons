...
public class ExpSolver {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (mc.currentScreen instanceof GuiChest) {
            ...
            if (Config.expersolver && chestName.contains("Ultrasequencer (")) {
    ...
    public void onGuiRender(ChestGui event) {
        if (Config.expersolver && event.displayName.contains("Ultrasequencer (")) {
            List<Slot> invSlots = event.slots;
            if (invSlots.size() > 48 && invSlots.get(49).getStack() != null) {
                if (invSlots.get(49).getStack().getDisplayName().contains("Timer: ")) {
