...
public class ChronomatronSolver {
    ...
    public void onSlotClick(ChestSlotClickedEvent event) {
        if (Config.expersolver && event.inventoryName.contains("Chronomatron (")) {
            IInventory inventory = event.inventory;
            ItemStack item = event.item;
            if (item == null) return;

            if (inventory.getStackInSlot(49).getDisplayName().contains("Timer: ") && (item.getItem() == Item.getItemFromBlock(Blocks.stained_glass) || item.getItem() == Item.getItemFromBlock(Blocks.stained_hardened_clay))) {
    ...
    public void onGuiRender(ChestGui event) {
        if (Config.expersolver && event.displayName.contains("Chronomatron (")) {
            int chestSize = event.chestSize;
            List<Slot> invSlots = event.slots;
            if (invSlots.size() > 48 && invSlots.get(49).getStack() != null) {
                if (invSlots.get(49).getStack().getDisplayName().contains("Timer: ") && invSlots.get(4).getStack() != null) {
                            ...
                            if (chronomatronMouseClicks + 1 < chronomatronPattern.size()) {
                                if (chronomatronPattern.get(chronomatronMouseClicks).equals(chronomatronPattern.get(chronomatronMouseClicks + 1))) {
                                    if (glass.getDisplayName().equals(chronomatronPattern.get(chronomatronMouseClicks))) {
                                        UtilsDanker.drawOnSlot(chestSize, glassSlot.xDisplayPosition, glassSlot.yDisplayPosition, CHRONOMATRON_NEXT + 0xE5000000);
                                    }
                                    ...
                                    UtilsDanker.drawOnSlot(chestSize, glassSlot.xDisplayPosition, glassSlot.yDisplayPosition, CHRONOMATRON_NEXT_TO_NEXT + 0XBE000000);
                                }
                            } else if (glass.getDisplayName().equals(chronomatronPattern.get(chronomatronMouseClicks))) {
                                UtilsDanker.drawOnSlot(chestSize, glassSlot.xDisplayPosition, glassSlot.yDisplayPosition, CHRONOMATRON_NEXT + 0xE5000000);
                            }
