    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (args.length > 0 && (args[0].startsWith("fail") || args[0].equals("f"))) {
            partyThread = new Thread(() -> {
                ...
                try {
                    ...
                    player.addChatMessage(new ChatComponentText(YungAddons.DELIMITER_COLOUR + "-----------------------------\n" +
                            YungAddons.MAIN_COLOUR + "Partying:" + EnumChatFormatting.WHITE + "\n- " +
                            EnumChatFormatting.YELLOW + members + "\n" +
                            YungAddons.DELIMITER_COLOUR + "-----------------------------"));
                    failInviting = true;
                    while (failInviting) {
                        Thread.sleep(10);
                    }
                    if (repartyFailList.size() > 0) {
                        ...
                        player.addChatMessage(new ChatComponentText(YungAddons.DELIMITER_COLOUR + "-----------------------------\n" +
                                YungAddons.MAIN_COLOUR + "Failed to invite:" + EnumChatFormatting.WHITE + "\n- " +
        ...
        partyThread = new Thread(() -> {
            ...
            try {
                ...
                player.addChatMessage(new ChatComponentText(YungAddons.DELIMITER_COLOUR + "-----------------------------\n" +
                        YungAddons.MAIN_COLOUR + "Repartying:" + EnumChatFormatting.WHITE + "\n- " +
                        EnumChatFormatting.YELLOW + members + "\n" +
                        YungAddons.DELIMITER_COLOUR + "-----------------------------"));
