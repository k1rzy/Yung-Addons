...
public abstract class TextOverlay {
    ...
    public void render() {
        ...
        render(overlayStringsSync, false);
    }
    ...
    protected Vector2f getSize(List<String> strings) {
        ...
        int paddingX = 0;
        int paddingY = 0;
        if(styleSupplier.get() == OverlayStyle.BACKGROUND) {
            paddingX = PADDING_X;
            paddingY = PADDING_Y;
        }
    ...
    private void render(List<String> strings, boolean dummy) {
        ...
        OverlayStyle style = styleSupplier.get();

        if(style == OverlayStyle.BACKGROUND) Gui.drawRect(x, y, x+overlayWidth, y+overlayHeight, 0x80000000);
        ...
        for(String s : strings) {
            if(s == null) {
                ...
                for(String s2 : s.split("\n")) {
                    ...
                    if(style == OverlayStyle.FULL_SHADOW) {
                    ...
                    Minecraft.getMinecraft().fontRendererObj.drawString(s2,
                            xPad, yPad, 0xffffff, style == OverlayStyle.MC_SHADOW);
